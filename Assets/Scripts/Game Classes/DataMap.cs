...
//		Debug.Log("Loading Data Map");
	...
	public void LoadData()
	{
		...
		LoadMap(File.ReadAllLines(dir+"/data.cfg"));
	}

	public void SaveData()
	{
		string dir = Application.dataPath;
		string[] dataMap = GetMap();
		Debug.Log(dir);
		if(File.Exists(dir+"/data.cfg"))
		{
			File.WriteAllLines(dir+"/data.cfg", dataMap);
		}
		else
		{
			StreamWriter dataFile = File.CreateText(dir+"/data.cfg");
			foreach(string l in dataMap)
			{
				dataFile.WriteLine(l);
			}
			dataFile.Flush();
			dataFile.Close();
		}
